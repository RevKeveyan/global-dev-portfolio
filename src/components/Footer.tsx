...
export const Footer = () => {
  ...
  return (
    <motion.footer 
      className="py-12 relative border-t border-border/30"
    ...
    >
      <div className="container-wide">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-8">
          ...
          <nav className="flex flex-wrap justify-center gap-6">
            {navLinks.map((link) => (
              <a
                key={link.key}
                href={link.href}
                className="text-sm text-muted-foreground hover:text-foreground transition-colors focus-ring rounded link-underline"
              >
                {t(`nav.${link.key}`)}
              </a>
            ))}
          ...
          <button
            onClick={scrollToTop}
            className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors focus-ring rounded p-2"
